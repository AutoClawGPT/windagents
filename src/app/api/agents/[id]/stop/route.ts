...
export async function POST(req: Request, ctx: Ctx) {
  ...
  if (cpk && remoteId) {
    try {
      const res = await clawpumpFetch(`/agents/${remoteId}/stop`, cpk, {
        method: "POST",
        body: "{}",
      });
      remote = await res.json().catch(async () => ({ status: res.status, raw: await res.text() }));
    } catch (e: unknown) {
      remote = { error: e instanceof Error ? e.message : "stop failed" };
    }
  } else if (!cpk) {
    return Response.json(
      {
        ...
        service: "ClawPump",
        message: "Save cpk_ in Settings to stop agents on ClawPump. Local status not faked without key.",
      },
      { status: 401 }
    );
  }
  ...
  return Response.json({ ok: true, agentId: localId, status: "stopped", clawpump: remote, canonicalId: localId });
}
